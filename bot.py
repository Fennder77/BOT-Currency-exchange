...
def start_command(message):
   bot.send_message(
       message.chat.id,
       'Приветствую! Могу показать курсы обмена ПриватБанка.\n' +
...
def help_command(message):
   ...
   keyboard.add(
       telebot.types.InlineKeyboardButton(
           "Message the developer", url='t.me/carenseBot'
       )
   )
   bot.send_message(
       message.chat.id,
       ...
       'курсах покупки и курсах продажи.\n' +
       '4) Нажмите “Обновить” чтобы получить актуальную информацию по запросу. ' +
       'The bot will also show the difference between the previous and the current exchange rates.\n' +
       '5) Бот поддерживает inline. Введите @carenseBot в любом чате и первые буквы валюты.',
...
def get_update_keyboard(ex):
    ...
    return keyboard
def serialize_ex(ex_json, diff=None):# разницы между текущим и старым обменным курсами при нажатии кнопки "Обновить".
...
def serialize_exchange_diff(diff):
   ...
   return result
def edit_message_callback(query):#Шаг No 8
...
@bot.inline_handler(func=lambda query: True)#Шаг 9
